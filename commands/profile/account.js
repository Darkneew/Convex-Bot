...
module.exports.action = (eventObject, args, dbUtils) => {
  switch (args.action) {
    case "create":
      ...
      break;
    case "change":
      let b = dbUtils.getUserAddress.get((eventObject.author || eventObject.user).id);
      if (b == -1) {
        eventObject.reply({embeds:[
          new EmbedBuilder()
    ...
    case "get":
      ...
      } else if (account.address == -1) eventObject.reply({embeds:[
        new EmbedBuilder()
        .setColor(parseInt(config.colors.error))
        .setTitle("Error")
        .setDescription(`You haven't created your account yet. Please use \`profile account create\` to make one.`)
      ]});
...
module.exports.buttons = {
  ...
  "existingaccount": (interaction, dbUtils, args) => {
    interaction.reply({embeds:[
      ...
      .addFields({"name":"Let me do it", "value":"If you let me do it, you will have to send your private key on discord"})
      .setColor(config.colors.question)
      .setFooter({text:"Convex", iconURL: config.links.logo })
    ], 
    components: [
  ...
  "newaccount": (interaction, dbUtils, args) => {
    interaction.reply({embeds:[
      ...
      .addFields({"name":"Let me do it", "value":"If you let me do it and want to retreive your private key, you will later either need me to send your private key on discord or download a small app to get it yourself."})
  ...
  "enterkeyswithoutaccount": (interaction, dbUtils, args) => {
    ...
    components: [
        new ActionRowBuilder() 
            ...
            .setStyle(ButtonStyle.Primary),
            new ButtonBuilder()
            .setURL("https://drive.google.com/file/d/1akmy0MTzFOoi6ycGKsleW_dS8DV3iEq8/view?usp=sharing")
            .setLabel("App")
  ...
  "enterkeyswithaccount": (interaction, dbUtils, args) => {
    ...
    components: [
        new ActionRowBuilder() 
            ...
            .setStyle(ButtonStyle.Primary),
            new ButtonBuilder()
            .setURL("https://drive.google.com/file/d/16uujXLUbXEJnfXCjOzeN0BYSUW81u2Yn/view?usp=sharing")
            .setLabel("App")
            .setStyle(ButtonStyle.Link),
            new ButtonBuilder()
  ...
  "createaccount": (interaction, dbUtils, args) => {
    interaction.reply({embeds:[
      ...
      .setDescription("To create an account, you first need to think of a password. A long and original password is highly recommanded. All characters are authorized. Putting in words from different languages is advised.")
      .setColor(config.colors.convex)
      .setFooter({text:"Convex", iconURL: config.links.logo })
    ], 
    components: [
  ...
  "generateinterkey": (interaction, dbUtils, args) => {
    interaction.reply({embeds:[
      new EmbedBuilder()
      .setTitle("Choose your password")
      .setDescription("To generate your interkey, you will need a password. A long and original password is highly recommanded. All characters are authorized. Putting in words from different languages is advised.")
      .setColor(config.colors.convex)
      .setFooter({text:"Convex", iconURL: config.links.logo })
    ], 
    components: [
...
module.exports.modals = {
  "filleverythingmodalaccount": (interaction, dbUtils, args) => {
    if (interaction.fields.getTextInputValue("interkey1") != interaction.fields.getTextInputValue("interkey2")) {
      let modal = new ModalBuilder()
      .setCustomId(`filleverythingmodalaccount`)
    ...
    if (interaction.fields.getTextInputValue("publickey1") != interaction.fields.getTextInputValue("publickey2")) {
      let modal = new ModalBuilder()
      .setCustomId(`filleverythingmodalaccount`)
    ...
    if (!argUtils.checkInt(interaction.fields.getTextInputValue("address1"), interaction, "address")) return;
    if (!argUtils.checkSize(interaction.fields.getTextInputValue("interkey1"), 64, interaction, "interkey")) return;
    if (!argUtils.checkSize(interaction.fields.getTextInputValue("publickey1"), 64, interaction, "public key")) return;
    dbUtils.setUserAccount.run(interaction.fields.getTextInputValue("address1"), interaction.fields.getTextInputValue("interkey1"), interaction.fields.getTextInputValue("publickey1"), interaction.user.id);
    interaction.reply({embeds:[
      new EmbedBuilder()
  ...
  "createeverythingaccount": async (interaction, dbUtils, args) => {
    if (interaction.fields.getTextInputValue("password1") != interaction.fields.getTextInputValue("password2")) {
      let modal = new ModalBuilder()
      .setCustomId(`createeverythingaccount`)
    ...
    let publicKey = ed25519.utils.bytesToHex(await ed25519.getPublicKey(cryptoUtils.hexToBytes(privateKey)));
    let interkey = cryptoUtils.generateInterkey(privateKey, interaction.fields.getTextInputValue("password1"));
    dbUtils.setUserAccount.run(address, interkey, publicKey, interaction.user.id);
    interaction.reply({embeds:[
      new EmbedBuilder()
  ...
  "interkeymodalaccount": async (interaction, dbUtils, args) => {
    if (interaction.fields.getTextInputValue("privatekey1") != interaction.fields.getTextInputValue("privatekey2")) {
      let modal = new ModalBuilder()
      .setCustomId(`interkeymodalaccount`)
    ...
    if (!argUtils.checkInt(interaction.fields.getTextInputValue("address1"), interaction, "address")) return;
    if (!argUtils.checkSize(interaction.fields.getTextInputValue("privatekey1"), 64, interaction, "private key")) return;
    let interkey = cryptoUtils.generateInterkey(interaction.fields.getTextInputValue("privatekey1"), interaction.fields.getTextInputValue("password1"));
    let publicKey = ed25519.utils.bytesToHex(await ed25519.getPublicKey(cryptoUtils.hexToBytes(interaction.fields.getTextInputValue("privatekey1"))));
    dbUtils.setUserAccount.run(interaction.fields.getTextInputValue("address1"), interkey, publicKey, interaction.user.id);
    interaction.reply({embeds:[
      new EmbedBuilder()
      .setTitle("Your account was successfully registered!")
      .setColor(config.colors.success)
